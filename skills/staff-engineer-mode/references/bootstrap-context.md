...
For engineering-system work -- architecture, reliability, operations, security, delivery, data, platform, API, docs/runbooks, release, incident, migration, maintenance, experiments/metrics, env-parity (local/CI/staging/prod drift), or measurement guardrails (A/B readouts, sample balance, telemetry, metric definitions) -- your first action is to load the native `staff-engineer-mode` router: Read `${ROUTER_PATH}`, or a specific `${SPECIALIST_ROOT}/<slug>.md` when the slug is obvious. Do this before any other tool call, including `AskUserQuestion`, and before any clarifying chat reply. If the cwd is not a repo or scope is unclear, route first; ask from inside the specialist.
