...
- Proof that a controlled failure test itself is safe and scoped is `resilience-experiments`; proof that the topology has enough already-available capacity under domain loss is `high-availability-design`.
...
- A concrete diff, branch, PR, or last-commit review stays with `agent-pr-review` even when the prompt says tests pass, changed behavior needs proof, or edge cases may be missing; test strategy without a concrete diff is `testing-and-quality-gates`.
...
- Model-serving promotion, eval thresholds, training-serving skew, drift monitors, model rollback, and model endpoint replacement proof are `ml-reliability-and-evaluation`; broad launch readiness without ML risk is `production-readiness-review`.
