...
8. **Check maintainer and surface.** Confirm changed files have a maintainer who would expect this change. Files touched outside the author's stated scope need an explicit reason or get flagged as out-of-scope.
...
11. **Classify findings.** For each finding, record category, evidence (file:line or behavior), maintainer, recommended next action, and risk level (blocker, must-fix-before-merge, follow-up, or accepted with rationale).
12. **Route specialist work.** Hand off security, database, rollout, observability, accessibility, or contract-evolution concerns to the right specialist instead of expanding scope here.
13. **Produce the structured artifact.** Output a single review with the categories below, not running prose. Reviewer reads this and can act without re-reading the diff.
...
- Start the artifact with an `Evidence anchors` line containing at least two changed `file:line` citations when the diff has two or more changed lines; one anchor may support intent, but blocker and must-fix findings still need their own cited evidence.
...
- Include at least two concrete diff anchors when the diff has enough changed lines: file:line citations, file:function references, or short quoted code excerpts. One anchor may support intent reconstruction; blocker and must-fix findings still need their own evidence.
...
- Explicit merge verdict: approve, request changes, or block, with reasons tied to observed issues or their absence.
