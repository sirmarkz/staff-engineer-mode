...
The source of truth for routing decisions is `skills/routing/staff-engineer-mode/SKILL.md`. Keep the full context map there so the router has enough always-loaded context to infer intent from realistic user prose.

This file records the highest-risk routing boundaries to preserve during future edits. Do not duplicate the full skill table here; duplication creates drift.
...
5. Ask one question when the artifact or phase is unclear.
...
- Build and artifact creation route to `release-engineering-and-build-reproducibility`; production exposure and rollback route to `progressive-delivery-and-safe-change`.
