...
- Encryption needs, key ownership, key separation, data classification, and long-lived confidentiality requirements.
...
8. **Audit and verify.** Emit access, privilege change, secret access, key use, and admin events with owner review.
...
- Legacy systems may need compensating controls while static credentials are removed; require owner, expiry, and rotation.
...
- Make recommendations actionable with owners, permission scopes, rotation steps, audit events, stop criteria, and migration gates where relevant.
- State required evidence such as access inventories, service identities, secret locations, key owners, rotation history, audit logs, and break-glass records; do not claim unseen evidence.
...
- `crypto_check`: cryptographic choices use vetted primitives and key ownership is defined.

## Red Flags - Stop And Rework

- Production access depends on network location alone.
- Long-lived shared secrets have no owner or rotation.
