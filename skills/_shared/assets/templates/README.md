...
| `api-contract.md` | `api-design-and-compatibility` | API contract | Keep compatibility, error shape, idempotency, and rollout fields aligned. |
...
| `backup-recovery-plan.md` | `backup-and-recovery` | Backup and restore plan | Keep RTO/RPO, restore evidence, owner, and recovery gaps visible. |
...
| `cache-derived-data-plan.md` | `caching-and-derived-data` | Cache and derived data plan | Keep freshness, invalidation, miss behavior, and repair path visible. |
| `cert-lifecycle-plan.md` | `cryptography-and-key-lifecycle` | Certificate and key lifecycle plan | Keep owner, expiry, overlap, verification, and rollback fields aligned. |
...
| `configuration-safety-review.md` | `configuration-and-automation-safety` | Configuration safety review | Keep validation, preview, blast radius, rollback, owner, and expiry fields aligned. |
...
| `data-pipeline-reliability.md` | `data-pipeline-reliability` | Data pipeline reliability plan | Keep freshness, validation, lineage, replay, and recovery fields visible. |
| `database-change-plan.md` | `database-operations` | Database change plan | Keep lock risk, rollback, throttling, query evidence, and owner fields aligned. |
...
| `distributed-data-consistency-plan.md` | `distributed-data-and-consistency` | Distributed data consistency plan | Keep consistency model, repair path, split-brain risk, and verification visible. |
| `documentation-lifecycle.md` | `documentation-lifecycle` | Documentation inventory and freshness plan | Keep source of truth, cadence, staleness signal, archive rule, and no-duplication rule visible. |
| `edge-traffic-defense-plan.md` | `edge-traffic-and-ddos-defense` | Edge traffic defense plan | Keep origin protection, limit behavior, customer impact, and abort path visible. |
...
| `event-workflow-contract.md` | `event-workflows` | Event workflow contract | Keep replay, ordering, duplicate handling, failed-message behavior, and owner visible. |
...
| `high-availability-design.md` | `high-availability-design` | High-availability design | Keep fault domains, static capacity, shared dependencies, and failover evidence visible. |
| `identity-and-secrets-review.md` | `identity-and-secrets` | Identity and secrets review | Keep scope, lifetime, rotation, break-glass path, and traceability fields aligned. |
...
| `infrastructure-policy-as-code-plan.md` | `infrastructure-and-policy-as-code` | Desired-state and policy plan | Keep drift detection, policy checks, reconciliation, exception, and rollback visible. |
| `internal-service-networking-plan.md` | `internal-service-networking` | Service networking plan | Keep discovery, identity, locality, access boundary, and fallback behavior visible. |
...
| `migration-deprecation-plan.md` | `migration-and-deprecation` | Migration and deprecation plan | Keep caller inventory, no-new-usage check, batches, rollback, and removal evidence visible. |
| `ml-reliability-readiness.md` | `ml-reliability-and-evaluation` | ML reliability readiness plan | Keep eval coverage, skew checks, drift checks, rollback, and production-risk fields aligned. |
| `mobile-release-plan.md` | `mobile-release-engineering` | Mobile release plan | Keep staged rollout, halt criteria, telemetry, rollback or forward-fix path visible. |
| `observability-alerting-spec.md` | `observability-and-alerting` | Observability and alerting spec | Keep metric definitions, missing-signal behavior, runbook, and owner visible. |
...
| `performance-capacity-plan.md` | `performance-and-capacity` | Performance and capacity plan | Keep load target, headroom, bottleneck, saturation signal, and action trigger visible. |
...
| `release-build-reproducibility.md` | `release-build-reproducibility` | Release reproducibility plan | Keep artifact identity, pinned inputs, promotion path, rollback traceability, and clean install evidence visible. |
...
| `rollout-plan.md` | `progressive-delivery` | Rollout plan | Keep canary metrics, stop criteria, owner, abort, rollback, and promotion pause visible. |
...
| `software-supply-chain-security.md` | `software-supply-chain-security` | Software supply chain security plan | Keep provenance, signing, dependency inventory, secret scan, vulnerability intake, and exception expiry visible. |
...
| `tenant-isolation-review.md` | `tenant-isolation` | Tenant isolation review | Keep boundary, shared resource, fallback path, access check, and residual risk visible. |
...
| `testing-quality-gates.md` | `testing-and-quality-gates` | Testing and quality gate plan | Keep merge blocker, release blocker, later signal, owner, and failure response visible. |
| `threat-model.md` | `secure-sdlc-and-threat-modeling` | Threat model | Keep trust boundaries, abuse cases, controls, residual risk, and verification visible. |
| `upgrade-readiness-matrix.md` | `fleet-upgrades` | Upgrade readiness matrix | Keep inventory, skew window, compatibility test, exception, and owner visible. |
...
| `web-release-gates.md` | `web-release-gates` | Web release gate plan | Keep loading, interaction readiness, layout stability, runtime error, and payload checks visible. |
