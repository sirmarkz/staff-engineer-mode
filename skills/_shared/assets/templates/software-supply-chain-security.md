# Software Supply Chain Security Plan

## Source-To-Deploy Map

| Stage | Actor | Input | Output | Control |
| --- | --- | --- | --- | --- |

## Provenance And Integrity

| Artifact | Source Revision | Accepted Change | Builder Identity | Dependency Inputs |
| --- | --- | --- | --- | --- |

| Artifact | Build Time | Confirmation Path | Verification Location |
| --- | --- | --- | --- |

## Dependency Inventory

| Dependency | Producer | Consumer | Retention | Vulnerability Checkpoint |
| --- | --- | --- | --- | --- |

## Credential Hardening

| Credential | Scope | Rotation | Storage | Audit |
| --- | --- | --- | --- | --- |

## Secret Scanning And Exposure Response

| Surface | Scanner/Signal | Exposure Response | Owner |
| --- | --- | --- | --- |

## Vulnerability Reporting Intake

| Finding Source | Triage Path | Handoff | SLA/Deadline |
| --- | --- | --- | --- |

## Exceptions

| Exception | Risk | Compensating Control | Expiry | Acceptance |
| --- | --- | --- | --- | --- |
