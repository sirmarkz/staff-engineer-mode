...
```
NO FAILURE EXPERIMENT WITHOUT HYPOTHESIS, BLAST RADIUS, ABORT CRITERIA, TELEMETRY, AND OWNER
```
...
- A team wants evidence that retry, failover, overload, backup, or dependency-failure behavior works.
...
- Participants, on-call coverage, communication channel, decision authority, and rollback/fallback actions.
...
3. **Bound the blast radius.** Start with shift-left simulation or staging when needed, then a small cell, tenant, shard, zone, or traffic slice.
...
5. **Prepare responders.** Confirm owner, on-call, runbooks, rollback, communication channel, and decision authority.
...
8. **Close the loop.** File fixes, update runbooks, add regression checks, and rerun only after material changes.
...
- Low-tier internal services may use lightweight drills when owners explicitly accept the risk.
...
- Make recommendations actionable with owners, exact fault injection, thresholds, stop authority, rollback commands, evidence to capture, and rerun criteria where relevant.
- State required evidence such as dashboards, SLO signals, deployment markers, runbooks, dependency health, experiment logs, findings, and fix owners; do not claim unseen evidence.
...
- Findings, fixes, owners, and rerun condition.

## Evidence Gates

- `hypothesis_check`: experiment states failure, expected behavior, and resilience mechanism.
- `blast_radius`: affected users, cells, tenants, shards, regions, or traffic percentage are bounded.
- `abort_criteria`: stop thresholds and decision authority are defined before the experiment.
- `telemetry_check`: steady-state and causal signals are visible during the test.
- `learning_loop`: findings create owned fixes or explicit risk acceptance.
...
- Findings are recorded but no fix owner or rerun condition exists.
