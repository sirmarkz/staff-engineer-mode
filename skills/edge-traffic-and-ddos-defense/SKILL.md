...
9. **Review and expire rules.** Temporary mitigations need owner, expiry, rollback, and post-event analysis.
...
- Include a compact rate-rule table for public APIs: route or route class, identity key (IP/session/user/tenant/API key), window, threshold, breach action, rollout mode, owner, and rollback.
