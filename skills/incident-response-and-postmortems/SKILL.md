...
12. **Replace single-root-cause wording with layered factors.** If the user supplies "root cause: X", treat X as one technical trigger, then add process, detection, rollout, ownership, or organizational defenses that allowed impact; mark inferred factors as candidates to verify.
13. **Create verified actions.** Every action needs owner, due date, observable completion signal, and classification: prevent, detect, mitigate, or learn.
14. **Feed standards.** Route recurring classes to SLOs, observability, safe change, HA, dependency resilience, or platform improvements.
...
- For postmortems, include a **Contributing Factors** section with at least three factors across at least two layers such as technical trigger, detection gap, rollout/process gap, ownership/runbook gap, or organizational tradeoff; avoid presenting one root cause as the whole explanation.
