...
- Concurrency/locking: singleton, leader election, or bounded parallelism; lease expiry and renewal; and a resource-enforced generation or fencing mechanism when a stale holder could still write.
...
3. **Prevent stale and overlapping writers.** Choose singleton execution, leader election, or bounded concurrency; decide whether a late run is skipped or queued when the prior run is still active. If ownership is lease-based, issue a monotonically increasing generation or fencing token on acquisition and require the protected state or side-effect boundary to reject operations from older holders. Define renewal, expiry, stop behavior, and reacquisition; a process-local belief that the lease is valid is not a safety boundary.
...
8. **Emit completion evidence safely.** Record run identifier, generation where used, start, end, outcome, and window covered so success is observable and gaps are countable. Keep raw job payloads, credentials, and personal data out of the run record unless a classified reproducibility need defines access, retention, redaction, and disposal.
...
- Make recommendations actionable with concrete trigger, lock or lease, resource-enforced fencing, deadline, alert, and catch-up decisions.
...
- Overlap/concurrency policy with lock or lease semantics, renewal/expiry/stop behavior, and resource-enforced generation or fencing when stale holders could mutate state.
...
- Completion-evidence and run-history record with sensitive-field, access, retention, and disposal rules.

## Checks Before Moving On

- `idempotent_run`: a repeated or double-fired run does not double-apply effects; partial progress resumes safely.
- `overlap_control`: singleton/leader/bounded concurrency is defined, with skip-or-queue behavior on overlap; lease-based ownership uses a resource-enforced generation or fencing mechanism where stale holders could write.
...
- An expired or paused lease holder can still mutate the protected resource after a newer holder acquires ownership.
...
| Trusting lease ownership alone | Fence stale holders at the protected resource with a monotonically increasing generation. |
