...
> This skill assumes a multi-surface engineering records request. The artifacts it produces (cross-surface control maps, engineering scorecards, exception registers using ISO/IEC 27005 and shared compensating-control fields) exist to coordinate records across different engineering surfaces. A solo developer can still use it when one project needs a combined record pack; single-domain records stay with the matching specialist skill.
...
Engineering controls are useful only when they are close to the work and produce records from engineering systems. Control mappings follow AICPA Trust Services Criteria 2017 control-objective format; risk-register format follows ISO/IEC 27005; compensating-control fields follow the shared compensating-control format; continuous monitoring follows NIST SP 800-37 Revision 2 RMF Step 7.
...
- The request explicitly spans two or more engineering surfaces and asks to design or collect one record pack, scorecard, control-to-artifact map, or exception register using ISO/IEC 27005 and shared compensating-control fields.
...
- Cross-surface engineering exceptions need expiry, compensating controls, residual risk, and revisit triggers in one register using ISO/IEC 27005, NIST SP 800-39, and the shared compensating-control format.
...
6. **Define exceptions.** Require expiry, compensating control, refresh trigger, and risk-acceptance authority appropriate to severity per NIST SP 800-39 and the shared compensating-control format.
...
- Lead with the expectation-to-record map, scorecard, exception register, or record pack outline requested, using ISO/IEC 27005 and the shared compensating-control format for exception fields.
...
- Exception register with expiry, compensating controls, refresh trigger, residual risk, and acceptance authority per ISO/IEC 27005, NIST SP 800-39, and the shared compensating-control format.
...
- `exception_check`: exceptions have expiry, compensating control, and refresh trigger per ISO/IEC 27005 and the shared compensating-control format.
...
| Open-ended exceptions | Add expiry, compensating control, and refresh trigger per ISO/IEC 27005 and the shared compensating-control format. |
