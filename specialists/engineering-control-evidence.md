...
- Existing artifacts: PRs, CI logs, tests, scans, build records, deployments, runbooks, incidents, access reviews, and dashboards, including their classification, access, retention, integrity, and disposal rules.
...
4. **Define record safety.** For each record class, specify the minimum evidence, prohibited fields, redaction or pseudonymization, access, retention and disposal, integrity, and volume bounds. Link to authoritative artifacts instead of copying raw secrets, personal data, incident content, logs, or source when a reference or digest proves the control.
5. **Locate records near engineering work.** Prefer minimized records generated by changes, CI, deploys, access systems, scanners, runbooks, and incidents.
6. **Assign responsibility and cadence.** Every record source needs an owner, refresh cadence, and failure response.
7. **Define exceptions.** Require expiry, compensating control, refresh trigger, and risk-acceptance authority appropriate to severity using the shared risk-acceptance lifecycle and compensating-control format.
8. **Build scorecards carefully.** Score capabilities and record state, not vanity metrics. Normalize overlapping security, reliability, supply-chain, operations, and internal engineering expectations into one record map.
9. **Create standards backlog.** Record gaps from failed record pulls, expired exceptions, incidents, and recurring findings with severity, expected fix path, and target date.
10. **Feed findings back.** Use incidents, failed reviews, and recurring exceptions to update standards and platform defaults.
...
- Scale the artifact to the request: return the cross-surface map and record-safety fields needed for the named expectations; add a scorecard, exception register, and standards backlog only when the user asks for those artifacts or gaps require them.
...
- Record inventory with source, classification, minimum evidence, prohibited fields, redaction, access, cadence, retention and disposal, integrity, and volume bounds.
...
- `record_safety`: each record class has minimization, prohibited-field, redaction, access, retention and disposal, integrity, and volume rules.
...
- A record pack copies raw secrets, personal data, incident content, logs, or source when a protected reference or digest would prove the same behavior.
